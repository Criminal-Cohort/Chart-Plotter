...
# Create top frame in Find Live tab
top_frame_find_live = tk.Frame(find_live_tab)
top_frame_find_live.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky='ew')

low_frame_find_live = tk.Frame(find_live_tab)
low_frame_find_live.grid(row=1, column=0, columnspan=2, padx=10, pady=10, sticky='e')

# Configure grid columns for the top frame
top_frame_find_live.grid_columnconfigure(0, weight=1)
top_frame_find_live.grid_columnconfigure(1, weight=1)
find_live_tab.grid_columnconfigure(0, weight=1)
find_live_tab.grid_columnconfigure(1, weight=1)

label_ticker = ttk.Label(top_frame_find_live, text="Ticker:", font=default_font)
label_ticker.grid(row=0, column=0, padx=(0, 10), pady=10, sticky='e')

entry_ticker = ttk.Entry(top_frame_find_live, font=default_font, width=15)
entry_ticker.grid(row=0, column=1, padx=(10, 0), pady=10, sticky='w')

checkbox_var_2 = tk.IntVar()
check_find_closed = tk.Checkbutton(low_frame_find_live, text="Find All", font=default_font, variable=checkbox_var_2)
check_find_closed.grid(row=1, column=0, padx=(0, 10), pady=10, sticky='w')

checkbox_var = tk.IntVar()
check_find_closed = tk.Checkbutton(low_frame_find_live, text="Find Closed", font=default_font, variable=checkbox_var)
check_find_closed.grid(row=1, column=1, padx=(0, 10), pady=10, sticky='w')

days_ticker = ttk.Label(low_frame_find_live, text="Days:", font=default_font)
days_ticker.grid(row=1, column=2, padx=(0, 10), pady=10, sticky='w')

entry_days = ttk.Entry(low_frame_find_live, font=default_font, width=15, )
entry_days.grid(row=1, column=3, padx=(10, 0), pady=10, sticky='w')
entry_days.insert(0, "33")

find_button = tk.Button(low_frame_find_live, text="Find", font=default_font, width=14, command=lambda: handle_find_live())
find_button.grid(row=2, column=3, padx=(10, 0), pady=5, sticky='e')

text_area_find_live = tk.Text(find_live_tab, font=default_font)
text_area_find_live.grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky='sew')
...
def handle_find_live():
    ...
    if checkbox_var_2.get():
        process_files(folder_path, extra=2)
    elif checkbox_var.get():
        process_files(folder_path, extra=1)
    else:
        process_files(folder_path)


def process_files(folder_path, extra=0):
    ...
    for file in files:
        ...
        try:
            ...
            final_day = np.busday_offset(datetime.today(), -33, roll='backward').astype('datetime64[D]').item()

            for idx, row in enumerate(sheet.iter_rows(min_row=3, values_only=True), start=1):
                if extra == 1:
                    if len(row) > 12 and (row[0] or row[11] or row[12]):
                        if row[1] == entry_ticker.get().upper():
                            print(row[2], final_day)
                            if datetime.strptime(str(row[2]).strip(), "%d-%b-%Y").date() >= final_day.date():
                                results += f"{idx + 2}, "
